...
class _Ticker:
    ...
    def history(self, **kwargs):
        ...
        return pd.DataFrame({"Close": [100.0, 101.0]}, index=pd.date_range("2026-01-01", periods=2))
...
class YfinanceCompatibilityTests(unittest.TestCase):
    ...
    def test_installed_public_surface_and_curl_metadata_are_compatible(self):
        ...
        for attribute in ("history", "info", "quarterly_income_stmt", "quarterly_balance_sheet", "quarterly_cash_flow"):
            self.assertTrue(hasattr(yfinance.Ticker, attribute), attribute)
        metadata = __import__("importlib.metadata", fromlist=["metadata"]).metadata("yfinance")
        curl_requirements = [
            Requirement(raw)
        ...
        if yfinance.__version__ != "1.5.2":
            self.skipTest(f"dependency environment has yfinance {yfinance.__version__}; pin check runs on supported environment")
        self.assertTrue(curl_requirements)
        self.assertTrue(any(req.specifier.contains("0.16.1") for req in curl_requirements))
    ...
    def test_download_public_interface_is_used_for_uninjected_history(self):
        ...
        prices = pd.DataFrame(
            {"BBCA.JK": np.linspace(100, 150, len(dates)), "BMRI.JK": np.linspace(80, 120, len(dates))},
            index=dates,
        )
        ...
        download.assert_called_once_with(
            ["BBCA.JK", "BMRI.JK"], period="3y", progress=False, auto_adjust=False, actions=True
        )
