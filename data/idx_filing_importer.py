...
def import_filings(
    ...
    *,
    ...
    try:
        if not repository.preflight_schema_migrations(["007_filing_work_ledger"])["ok"]:
    ...
    def record(row, state, action, code, counter):
    ...
    for row in prepared:
        ...
        try:
            # The database clock is authoritative for the lease before network.
            if not repository.renew_filing_work(row, token):
                record(row, "RUNNING", "DEFERRED", "LEASE_EXPIRED", "leased_elsewhere")
                continue
            ...
            if row.get("checksum") and artifact["checksum"] != row["checksum"]:
                error_class, code = "PROVENANCE", "ARTIFACT_MISMATCH"
            ...
            try:
                ...
                completed = repository.complete_filing_import(
                    row, token, artifact, state=outcome
                )
            if not completed:
                record(row, "RUNNING", "DEFERRED", "LEASE_EXPIRED", "leased_elsewhere")
                continue
            record(
                ...
                "ACCEPTED" if outcome == "ACCEPTED" else "VALIDATION_FAILED",
            ...
            try:
                durable = repository.finalize_filing_work(
                    row, token, "RETRYABLE", error_class=error_class, error_summary=code
                )
            except Exception:
                code = "DATABASE_UNAVAILABLE"
            record(
