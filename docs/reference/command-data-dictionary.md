...
| `inspect-candidate-readiness` | operations/storage | candidate snapshot → strict per-Issuer readiness JSON; exits 2 when existing readiness fails | exact blocker tickers, unchanged gate booleans, point-in-time concept/history inventory, official source identities, and checksums; malformed fields fail closed without echo | [`tests/test_readiness_diagnostics.py`](../../tests/test_readiness_diagnostics.py) |
...
| `run-daily-research` | operations/analysis | release, database, optional R2, final-attempt → run report and publication attempts | release ID, formula version, evidence cutoff, snapshot checksum; rejected quant readiness includes the same per-Issuer diagnostic object in persisted metrics and the job summary; quant may publish before a later scan failure | [`tests/test_research_automation.py`](../../tests/test_research_automation.py); no direct CLI seam |
