...
| --- | --- | --- | --- | --- |
| `ingest-manifest` | `data`, operations | reviewed source manifest → JSON report | source URL, checksum, published/retrieved times; parse failures quarantine | [`tests/test_core_v3.py`](../../tests/test_core_v3.py); no direct CLI seam |
...
| `refresh-market-history` | operations/storage | source manifest and provider policy → market report | completed session, provider, and availability; failure returns non-ready report | [`tests/test_scan_v2.py`](../../tests/test_scan_v2.py); no direct CLI seam |
