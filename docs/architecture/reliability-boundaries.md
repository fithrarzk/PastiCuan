...
| --- | --- | --- |
| Evidence | [Acquisition](../../data/ingestion.py) retrieves/checksums artifacts; [validation](../../data/validation.py) and [XBRL parsing](../../data/idx_xbrl.py) classify or reject them; [CLI ingestion](../../operations/research_cli.py) and the [repository](../../storage/repository.py) persist quarantine status/issues; repository point-in-time queries select `available_at <= as_of` and non-quarantined rows | Missing or conflicting official evidence yields degraded/unavailable output; fallback is disclosed and does not open action gates |
