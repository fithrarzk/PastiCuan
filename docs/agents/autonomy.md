...
- edit code, tests, documentation, workflows, and additive migrations within a lane-appropriate task note or card;
- push task branches, request review, address feedback, and rerun failed deterministic checks;
- squash-merge when branch rules, task policy, lane-required review, and current-head checks allow it;
...
Autonomy increases evidence requirements. Every autonomous task must leave its
lane-appropriate task note or card, commits, PR discussion, current-head checks,
required review, relevant deployment/result identifiers, and a structured
handoff. Failure retains the last-good release and creates one actionable
incident; it never fabricates completion.
