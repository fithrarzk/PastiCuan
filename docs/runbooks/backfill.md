...
Stop on a non-official URL, identity conflict, missing profile, quarantine affecting required coverage, stale data, failed signing, unavailable scan, or a writer connection that cannot preserve a session advisory lock. ING-003 code makes imports skip-before-download resumable only where reviewed migrations 007 and 008 are applied with exact identity and grants. Until their protected production rollout is proven, production import remains unavailable and this local/CI behavior is not production truth. Cumulative discovery merge is available under ING-001.
